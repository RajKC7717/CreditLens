...
if not st.session_state.analyzed:
    ...
    **How to use:** Select a sample profile from the sidebar or enter customer details, then click **Analyze Customer**.
    ...
    signals = [
        ("Monthly Inflow", f"₹{profile['monthly_inflow_avg']:,.0f}"),
        ("Bill Punctuality", f"{profile['utility_avg_days_late']:+.1f} days"),
        ("Essential Spend", f"{profile['essential_spend_ratio']*100:.0f}%"),
        ("COD Usage", f"{profile['ecommerce_cod_ratio']*100:.0f}%"),
        ("Bills Missed", str(int(profile['utility_missed_count'])))
    ]
    ...
    st.markdown("*Adjust sliders to see how behavior changes affect the score in real time*")

    sim_col1, sim_col2, sim_col3 = st.columns(3)
    with sim_col1:
        sim_bills_late = st.slider("📅 Utility bills days late",
            -10.0, 30.0, float(profile["utility_avg_days_late"]), 0.5, key="sim_bills")
    with sim_col2:
        sim_missed = st.slider("🧾 Missed bills",
            0, 10, int(profile["utility_missed_count"]), 1, key="sim_missed")
    with sim_col3:
        sim_cod = st.slider("📦 COD usage",
            0.0, 1.0, float(profile["ecommerce_cod_ratio"]), 0.05, key="sim_cod")
    ...
    with sim_col_b:
        st.metric("Simulated Score", f"{sim_score} / 100", delta=f"{score_diff:+d} points")
    with sim_col_c:
    ...
    if score_diff > 0:
        st.success(f"✅ If {customer_name} makes these changes, score improves by **{score_diff} points** → {sim_result['risk_category']}")
    elif score_diff < 0:
